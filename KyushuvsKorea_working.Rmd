...
theme_PubOld <- function(base_size=12, base_family="arial") {
      library(grid)
      library(ggthemes)
   (theme_foundation(base_size=base_size, base_family=base_family)
 + theme_bw()+
...
#3c.Create concentration dependent mixing lines for plot in case needed. 
...
#3d.Plot isotope data from pots against reference ranges
...
p_Kyu <-ggplot()+
  ...
  geom_star (data= subset(Kyu, Aquatic =="yes"), aes(y=d13C18, x=d13C16), shape=21, fill= "blue", size=3, colour="black")+
geom_star (data= subset(Kyu, Aquatic =="Part"), aes(y=d13C18, x=d13C16), shape=21,  fill= "lightblue", size=3, colour="black")+
  geom_point(data=subset(Kyu, Aquatic=='no'), aes(y=d13C18,x=d13C16), shape=21, fill="grey", size=3, colour ="black")+ 
  stat_ellipse(geom= "polygon", data=Iso_ref, alpha=0.1,
...
p + facet_wrap (~factor(Phase_2, levels=c('Mumun', 'Jomon', 'Yayoi')))
```
```{r}
library(scales)
Rice_pots <-Data_pots %>% filter (Category == "Rice" )
...
p <-ggplot()+
  ...
  scale_y_continuous(limits=c(-37,-15))+
     stat_ellipse(geom= "polygon", data=Iso_ref, alpha=0.2,
               aes (x = d13C16, y = d13C18, group = Category),
              level = 0.68)+
annotate("text", label = "C4/millet", x = -20, y = -17, size = 2, colour = "black")+ 
  annotate("text", label = "Marine", x = -25, y = -22, size = 2, colour = "black")+ 
  annotate("text", label = "Freshwater", x = -31, y = -29, size = 2, colour = "black")+ 
  annotate("text", label = "Wild boar", x = -30, y = -27, size = 2, colour = "black")+ 
  annotate("text", label = "Ruminant", x = -29, y = -35, size = 2, colour = "black")+ 
  annotate("text", label = "C3 nuts", x = -35, y = -33, size = 2, colour = "black")+
  annotate("text", label = "Rice", x = -34, y = -34, size = 2, colour = "black")+   coord_fixed(ratio = 1)
p
```
...
p1 <-p +
    geom_point(data=subset(Data, Millet=='no'), aes(y=d13C18,x=d13C16), shape=21, size=2, colour ="#999999")+ 
  geom_point(data=subset(Data, Millet=='n/a'), aes(y=d13C18,x=d13C16), shape=21, size=2, colour ="#999999")+
  geom_point (data= subset(Data, Millet=="yes"), aes(y=d13C18, x=d13C16), shape=21, fill= "#E69F00", size=2, colour="black")+
  theme(axis.ticks = element_blank())+
  theme_bw(base_size = 10)+
  theme(strip.text.x = element_blank())
p1 <- p1 + facet_wrap (~factor(Phase_2, levels=c('Mumun', 'Jomon', 'Yayoi')))
p1
```
...
```{r}
p2 <-p +
    geom_point(data=subset(Data, Aquatic=='no'), aes(y=d13C18,x=d13C16), shape=21, size=2, colour ="#999999")+ 
  geom_point(data=subset(Data, Aquatic=='n/a'), aes(y=d13C18,x=d13C16), shape=21, size=2, colour ="#999999")+ 
  geom_point (data= subset(Data, Aquatic=="yes"), aes(y=d13C18, x=d13C16), shape=21, fill= "#56B4E9", size=2, colour="black")+
    theme_bw(base_size = 10)+
  theme(panel.background = element_blank())+
    #theme(axis.ticks = element_blank())+
  theme(strip.text.x = element_blank())+
  facet_wrap (~factor(Phase_2, levels=c('Mumun', 'Jomon', 'Yayoi'))) 
p2  
```
...
Try one plot for all biomarkers. 
...
p3 <-p +
  ...
  theme_bw(base_size = 10)+
  theme(strip.text.x = element_blank())
p3 <- p1 + facet_wrap (~factor(Phase_2, levels=c('Mumun', 'Jomon', 'Yayoi')))
p3
```
...
```{r}
p4 <- p +
      geom_point(data = Rice_pots, aes(y=d13C18, x=d13C16,fill=p95),pch= 21, size=2)+
...
```{r}
p5 <- p +
      geom_point(data = Millet_pots, aes(y=d13C18, x=d13C16,fill=p95),pch= 21, size=2)+
    scale_fill_gradient (low="blue", high ="orange", limits = c(0,1))+
  #labs(fill = "Millet") +
  theme_bw(base_size = 10)+
  theme(panel.background = element_blank())+
...
```{r}
p6 <- p +
      geom_point(data = Marine_pots, aes(y=d13C18, x=d13C16,fill=p95),pch= 21, size=2)+
    scale_fill_gradient (low="blue", high ="orange", limits = c(0,1))+
  labs(fill = "Marine") +
  theme_bw(base_size = 10)+
  theme(panel.background = element_blank())+
...
```{r}
p7 <- p +
      geom_point(data = RUM_pots, aes(y=d13C18, x=d13C16,fill=p95),pch= 21, size=2)+
    scale_fill_gradient (low="blue", high ="orange", limits = c(0,1))+
  #labs(fill = "Ruminant") +
  theme_bw(base_size = 10)+
  theme(panel.background = element_blank())+
